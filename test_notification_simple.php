...
// This will help us verify our simplified approach
...
function createNotification($playerId, $gameId, $notificationType, $message = null) {
    ...
    echo "Creating notification for player $playerId, game $gameId, type $notificationType at $now\n";
...
function getNewNotifications($writerId, $lastCheck = null) {
    ...
    if ($lastCheck !== null) {
        echo "Comparing notification created_at ({$notification['created_at']}) with lastCheck ($lastCheck)\n";
        
        // Simple string comparison (works because both are in the same format)
        if ($notification['created_at'] <= $lastCheck) {
            echo "Notification was created before or at lastCheck, not returning it\n";
            return [];
        }
    }
...
// Change the server timezone (simulating a different server)
...
echo "Restored server timezone to: " . date_default_timezone_get() . "\n";
?> 